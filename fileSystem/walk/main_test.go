...
func createTempDir(t *testing.T, files map[string]int) (dirname string, cleanup func()) {
	...
	for k, n := range files { // map extension to number of files eg .log: 4, .go: 3 etc
		for j := 1; j <= n; j++ {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if len(filesLeft) != tc.nNoDelete {
				t.Errorf("Expted %d files left, got %d instead\n", tc.nNoDelete, len(filesLeft))
			}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if len(filesArchived) != tc.nArchive {
				t.Errorf("Expected %d files archives, got %d instead\n", tc.nArchive, len(filesArchived))
			}
		})
	}
}
